...
For Bash-based workflows, OpenBrowser provides a `-c` flag that connects to a persistent browser daemon over a Unix socket. No MCP server required.
...
uvx openbrowser-ai -c "results = await evaluate('document.querySelectorAll(\"h2\").length')"
uvx openbrowser-ai -c "print(results)"

# Daemon management
uvx openbrowser-ai daemon start     # Start (auto-starts on first -c call)
uvx openbrowser-ai daemon stop      # Stop daemon and browser
uvx openbrowser-ai daemon status    # Show daemon info
uvx openbrowser-ai daemon restart   # Restart with fresh session
```

The daemon starts automatically on first `-c` call and shuts down after 10 minutes of inactivity. It uses a Unix socket at `~/.openbrowser/daemon.sock`.
